...
module.exports = functions.https.onRequest(async (request, response) => {
  cors(request, response, async () => {
    ...
    const displayScheme = request.headers.display_scheme;
    ...
    if (
      ...
      response.send(
        `user_id, label_session_id, rate_id, success_url, and cancel_url, display_scheme headers must exist on the request`
      );
      return;
    }
    ...
    try {
      await db
        ...
        .collection("label-sessions")
        .doc(labelSessionID)
        .update({ lastRate: rate, displayScheme });
    } catch {
      response.send({
